...
bit  full_data[L]; byte a=0; byte b=0; byte c=0; byte d=0; byte z=0;  bit my_pswd[N];
...
end1:
 do
 ::z<N->my_pswd[z] =  full_data[z] || (full_data[z+2] && full_data[z+4]);    
 progress1: to_receiver ! my_pswd[z];
z++;
 od; 
}
 proctype receiver (chan from_A, from_B, from_C)
 { bit pswd[L] ; byte w=0; byte id=0; byte psw;
do
:: from_A ? psw -> id=0;
                                 if
                                  ::(w<N)-> pswd[id*N+w]=psw;w++
                                 ::(w>=N) ->break
                                 fi
od
